...
class RedisServiceSimple:
    ...
    async def criar_produto(self, dados_produto: Dict[str, Any]):
        ...
        try:
            ...
            produto_existente = await self.db.produtos.find_one({"nome": dados_produto["nome"]})
            ...
            logger.info(f"Produto criado com sucesso: {dados_produto['nome']} (ID: {result.inserted_id})")
        except Exception as e:
    ...
    async def atualizar_produto(self, produto_id: int, dados_produto: Dict[str, Any]):
        ...
        try:
            ...
            produto = await self.db.produtos.find_one({"nome": dados_produto["nome"]})
            if produto:
                dados_produto["updated_at"] = datetime.now()
                await self.db.produtos.update_one(
                    {"nome": dados_produto["nome"]}, 
                    {"$set": dados_produto}
                )
                logger.info(f"Produto atualizado com sucesso: {dados_produto['nome']}")
            else:
    ...
    async def deletar_produto(self, produto_id: int):
        ...
        try:
            # Buscar produto por nome (assumindo que o nome contém o ID)
